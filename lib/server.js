...
server.unifiedServer = function(req,res){
    ...
    let parsedUrl = new url.URL(req.url, "http://localhost:3000/");
    ...
    let queryStringObject = new url.URLSearchParams(parsedUrl.searchParams);
    ...
    req.on('end',()=>{
        ...
        chosenHandler(data,function(statusCode,payload,contentType){
            ...
            if(contentType == 'favicon'){
                ...
                //set contentType header to favicon
                res.setHeader('Content-Type','image/x-icon');
            }
            if(contentType == 'css'){
                //use payload called by handler, or default to empty string
                payloadString = typeof(payload) !== 'undefined' ? payload : '';
                //set contentType header to css support
                res.setHeader('Content-Type','text/css');
            }
            if(contentType == 'png'){
                //use payload called by handler, or default to empty string
                payloadString = typeof(payload) !== 'undefined' ? payload : '';
                //set contentType header to png support
                res.setHeader('Content-Type','image/png');
            }
            if(contentType == 'jpg'){
                //use payload called by handler, or default to empty string
                payloadString = typeof(payload) !== 'undefined' ? payload : '';
                //set contentType header to jpg support
                res.setHeader('Content-Type','image/jpeg');
            }
            if(contentType == 'plain'){
                //use payload called by handler, or default to empty string
                payloadString = typeof(payload) !== 'undefined' ? payload : '';
                //set contentType header to plain 
